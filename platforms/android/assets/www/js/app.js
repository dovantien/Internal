angular.module('starter', ['ionic', 'ngCordova', 'ionic.cloud', 'ngAudio', 'starter.controllers', 'starter.services.APIService', 'starter.services.UtilsService', 'starter.services.UserService',
    'starter.services.ClosePopupService', 'starter.services.PopupService', 'tabSlideBox', 'starter.services.WSService'
])

.run(function($ionicPlatform, $ionicDeploy, APIService, UserService, $q, $cordovaBarcodeScanner,$ionicLoading) {
                ...
                $ionicDeploy.getMetadata().then(function(metadata) {
                    if (parseInt(metadata.version) > DEPLOY_VERSION) {
                        console.log('version cũ');
                            $ionicLoading.show({
                                template: '<ion-spinner icon="bubbles"></ion-spinner><p>Đang Cập Nhật Phiên Bản Mới </p>',
                                content: 'Loading',
                                // animation: 'fade-in',
                                showBackdrop: true,
                                maxWidth: 250,
